...
from examples.omni_adapter.tts_server import (
    PersistentTTSWorker,
    TTSError,
    _command,
    _synthesis_spec,
...
def test_persistent_tts_patch_recreates_audio_helper_per_prompt() -> None:
    patch = Path(
        "training_suite/patches/llama.cpp-qwen3tts-persistent.patch"
    ).read_text()
...
def test_silent_video_returns_no_audio_instead_of_failing(monkeypatch) -> None:
    ...
    monkeypatch.setattr("examples.omni_adapter.server.subprocess.run", run)
    media = MediaItem(
        kind="video",
        mime_type="video/mp4",
        data=_mp4(),
